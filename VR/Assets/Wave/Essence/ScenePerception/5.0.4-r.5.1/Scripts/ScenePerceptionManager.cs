...
namespace Wave.Essence.ScenePerception
{
	public class ScenePerceptionManager : MonoBehaviour
	{
		...
		/// </summary>
		public WVR_Result GetSceneMeshes(WVR_ScenePerceptionTarget perceptionTarget, out WVR_SceneMesh[] meshes)
		{
			UInt32 meshCount = 0;
			meshes = new WVR_SceneMesh[1]; //Empty array

			WVR_Result result = Interop.WVR_GetSceneMeshes(perceptionTarget, 0, out meshCount, IntPtr.Zero); //Get mesh count
			if (result != WVR_Result.WVR_Success)
			{
			...
			result = Interop.WVR_GetSceneMeshes(perceptionTarget, meshCount, out meshCount, meshesPtr); //Get meshes
			if (result != WVR_Result.WVR_Success)
			{
		...
		public void ApplyTrackingOriginCorrectionToPlanePose(WVR_ScenePlane scenePlane, out Vector3 planePosition, out Quaternion planeRotation)
		{
			...
			{
				Matrix4x4 trackingSpaceOriginTRS = Matrix4x4.TRS(trackingOrigin.transform.position, trackingOrigin.transform.rotation, Vector3.one);
				Matrix4x4 worldSpaceAnchorPoseTRS = Matrix4x4.TRS(planePosition, planeRotation, Vector3.one);

				Matrix4x4 trackingSpaceAnchorPoseTRS = trackingSpaceOriginTRS * worldSpaceAnchorPoseTRS;

				planePosition = trackingSpaceAnchorPoseTRS.GetColumn(3); //4th Column of TRS Matrix is the position
				planeRotation = Quaternion.LookRotation(trackingSpaceAnchorPoseTRS.GetColumn(2), trackingSpaceAnchorPoseTRS.GetColumn(1));
			}
		}
